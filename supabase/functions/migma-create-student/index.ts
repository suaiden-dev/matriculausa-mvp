...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Inserir perfil com source='migma' e placement_fee_flow=true
    const { data: profile, error: profileError } = await supabase
      ...
      .insert({
        ...
        role: 'student',
        status: 'active',
        placement_fee_flow: true, // novo fluxo: Placement Fee em vez de Scholarship + I-20
      })
    ...
    return new Response(
      JSON.stringify({ success: true, user_id: userId, profile_id: profile.id, profile }),
