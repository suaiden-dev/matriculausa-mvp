-- Migration: Fix get_student_detailed_info function JOIN issue
-- This fixes the problem where the function was trying to JOIN using user_id instead of profile_id

-- Drop and recreate the function with proper JOIN logic
...
BEGIN
  ...
  seller_info AS (
    ...
    FROM sellers s
    WHERE s.referral_code = (SELECT seller_referral_code FROM student_profile)
  ),
  payment_info AS (
    SELECT 
  ...
  scholarship_info AS (
    ...
    WHERE sa.student_id = (SELECT profile_id FROM student_profile)
  ...
  SELECT 
    ...
    sp.documents_status,
    sp.is_application_fee_paid,
    sp.is_scholarship_fee_paid,
    sp.has_paid_selection_process_fee,
    sp.has_paid_i20_control_fee,
    COALESCE(sp.student_process_type, 'Not specified') as student_process_type,
  ...
  LEFT JOIN scholarship_applications sa ON sa.student_id = sp.profile_id
  ORDER BY sa.created_at DESC
  LIMIT 1;
END;
