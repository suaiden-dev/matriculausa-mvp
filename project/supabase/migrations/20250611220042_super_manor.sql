...
BEGIN
  ...
  IF user_email IS NULL OR user_email = '' THEN
    RETURN json_build_object('success', false, 'message', 'Email cannot be empty');
  END IF;
  ...
  IF target_user_id IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'User with email ' || user_email || ' not found');
  END IF;
  ...
  result_json := json_build_object(
    'success', true, 
    'message', 'User ' || user_email || ' has been promoted to admin',
