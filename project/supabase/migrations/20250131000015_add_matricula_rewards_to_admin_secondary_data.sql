-- Migration: Add Matricula Rewards referral info to get_admin_student_secondary_data
-- Description: Adiciona informações sobre quem indicou o aluno usando código MATR à RPC existente
...
DECLARE
  ...
  matricula_rewards_info jsonb; -- Nova variável para Matricula Rewards
BEGIN
      ...
      SELECT COALESCE(
        ...
        COALESCE(
          (
            ...
            LEFT JOIN user_profiles up ON urc.referrer_id = up.user_id
          ...
          COALESCE(
            (
              ...
              LEFT JOIN user_profiles up ON ar.referrer_id = up.user_id
  ...
  -- 4. NOVO: Buscar informações do Matricula Rewards (quem indicou este aluno usando código MATR)
  -- Buscar se o aluno atual usou algum código MATR e quem o indicou
  SELECT COALESCE(
    (
      SELECT jsonb_build_object(
        'code', urc.affiliate_code,
        'used_at', urc.created_at,
        'referrer_id', urc.referrer_id,
        'referrer_name', COALESCE(referrer_up.full_name, NULL),
        'referrer_email', COALESCE(referrer_up.email, NULL)
      )
      FROM used_referral_codes urc
      LEFT JOIN user_profiles referrer_up ON referrer_up.user_id = COALESCE(urc.referrer_id, (
        SELECT ac.user_id 
        FROM affiliate_codes ac 
        WHERE ac.code = urc.affiliate_code 
        LIMIT 1
      ))
      WHERE urc.user_id = target_user_id
        AND urc.affiliate_code LIKE 'MATR%'
      ORDER BY urc.created_at DESC
      LIMIT 1
    ),
    'null'::jsonb
  ) INTO matricula_rewards_info;
  ...
  SELECT jsonb_build_object(
    ...
    'matricula_rewards_info', matricula_rewards_info -- Adicionar Matricula Rewards info
  ) INTO result;
...
'Consolida todas as queries secundárias de AdminStudentDetails (term acceptances, referral info, real paid amounts, matricula rewards info) em uma única chamada RPC, reduzindo múltiplas queries para 1 request.';
