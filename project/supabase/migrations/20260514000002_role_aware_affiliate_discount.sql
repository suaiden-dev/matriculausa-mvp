/*
  Torna o desconto de indicação ciente do papel (role) do referenciador:
  - role = 'affiliate' (Programa Torne-se Afiliado) → discount_amount = 0
  - role = 'student' (Matricula Rewards automático)  → discount_amount = 50
*/
...
BEGIN
  ...
  -- Determinar desconto com base no papel do referenciador
  SELECT role INTO referrer_role
  FROM user_profiles
  WHERE user_id = referrer_user_id
  LIMIT 1;

  -- Apenas alunos (Matricula Rewards automático) concedem $50 de desconto.
  -- Afiliados profissionais (role = 'affiliate') não concedem desconto.
  IF referrer_role = 'student' THEN
    v_discount_amount := 50.00;
  ELSE
  ...
  INSERT INTO used_referral_codes (
    ...
    user_id_param, normalized_code, referrer_user_id, v_discount_amount,
    stripe_coupon_id, 'applied', now(), email_param
  );

  -- Criar registro em affiliate_referrals
