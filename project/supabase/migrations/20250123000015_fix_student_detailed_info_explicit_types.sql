-- Migration: Fix get_student_detailed_info function types - explicit casting
-- This fixes the column 28 type mismatch error by ensuring all numeric types are properly cast
...
BEGIN
  ...
  scholarship_info AS (
    SELECT 
      ...
      COALESCE(sa.documents_status, 'pending') as documents_status
  ...
  SELECT 
    ...
    COALESCE(schi.documents_status, sp.documents_status),
...
-- Add comment
COMMENT ON FUNCTION get_student_detailed_info(uuid) IS 'Fixed function with explicit type casting to resolve column 28 type mismatch error';