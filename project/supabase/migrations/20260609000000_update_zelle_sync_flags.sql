-- Migration: Update Zelle sync flags function to support package installments
-- Created: 2026-06-09
-- Description: Recreates sync_user_profile_payment_flags() to check if a package installment plan is active and avoid setting flags prematurely.
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    ELSIF NEW.fee_type = 'ds160_package' THEN
      ...
      -- Verificar se o pagamento conclui o parcelamento (se houver plano ativo)
      SELECT COALESCE(
        (SELECT (installments_paid + 1 >= total_installments OR amount_paid + amount_param >= total_amount)
         FROM fee_installment_plans 
         WHERE user_id = NEW.user_id AND fee_type = 'ds160_package' AND status = 'active'),
        true
      ) INTO is_completed;
    ...
    ELSIF NEW.fee_type = 'i539_cos_package' THEN
      ...
      SELECT COALESCE(
        (SELECT (installments_paid + 1 >= total_installments OR amount_paid + amount_param >= total_amount)
         FROM fee_installment_plans 
         WHERE user_id = NEW.user_id AND fee_type = 'i539_cos_package' AND status = 'active'),
        true
      ) INTO is_completed;
