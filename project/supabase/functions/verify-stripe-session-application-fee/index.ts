...
Deno.serve(async (req) => {
  console.log(
    "--- verify-stripe-session-application-fee: Request received ---",
  );
  ...
  try {
    ...
    if (existingLog) {
      console.log(
        `[DUPLICAÇÃO] Session ${sessionId} já foi processada, retornando sucesso sem reprocessar.`,
      );
      return corsResponse({
        status: "complete",
    ...
    if (session.payment_status === "paid" && session.status === "complete") {
      ...
      const paymentMethodForApplication = isPixPayment
        ? "pix"
        : (session.metadata?.payment_method || "stripe"); // 'pix' ou 'stripe' para scholarship_applications

      // Variável para lógica de conversão (usada para detectar PIX)
      const paymentMethod = isPixPayment
        ? "pix"
        : (session.metadata?.payment_method || "stripe");

      console.log(
        `Processing successful payment. UserID: ${userId}, ApplicationID: ${applicationId}, Payment Method: ${paymentMethodForApplication}`,
      );
      if (!userId) {
        return corsResponse({
      ...
      const { data: userProfile, error: profileError } = await supabase.from(
        "user_profiles",
      ).select("id, user_id").eq("user_id", userId).single();
      ...
      console.log(
        `User profile found: ${userProfile.id} for auth user: ${userId}`,
      );
      ...
      try {
        await supabase.rpc("log_student_action", {
          ...
          p_action_description:
            `Application Fee payment processing started (${sessionId})`,
      ...
      const { data: application, error: fetchError } = await supabase.from(
        "scholarship_applications",
      ).select("id, student_id, scholarship_id, student_process_type, status")
        .eq("id", applicationId).eq("student_id", userProfile.id).single();
      ...
      console.log(
        `[verify-stripe-session-application-fee] Current application status: '${application.status}' for user ${userId}, application ${applicationId}.`,
      );
      if (application.status !== "approved") {
        updateData.status = "under_review";
        console.log(
          `[verify-stripe-session-application-fee] Application status set to 'under_review' for user ${userId}, application ${applicationId}.`,
        );
      } else {
        console.log(
          `[verify-stripe-session-application-fee] Preserving 'approved' status for user ${userId}, application ${applicationId} (university already approved).`,
        );
      }
      // Se student_process_type não existe na aplicação, tentar obter dos metadados da sessão
      if (
        !application.student_process_type &&
        session.metadata?.student_process_type
      ) {
        updateData.student_process_type = session.metadata.student_process_type;
        console.log(
          "Adding student_process_type from session metadata:",
          session.metadata.student_process_type,
        );
      }
      // Atualiza a aplicação
      console.log(
        `Updating application ${applicationId} with data:`,
        updateData,
      );
      ...
      const { data: updatedApplication, error: verifyError } = await supabase
        ...
        .select(
          "id, application_fee_payment_method, is_application_fee_paid, payment_status",
        )
        .eq("id", applicationId)
        .single();
      ...
      if (updateData.status) {
        console.log(
          `Application status updated to '${updateData.status}' with payment info`,
        );
      } else {
      ...
      const { data: userProfileDocs, error: userProfileError } = await supabase
        .from("user_profiles").select("documents").eq("user_id", userId)
        .single();
      if (userProfileError) {
        console.error(
          "Failed to fetch user profile documents:",
          userProfileError,
        );
        ...
        if (formattedDocuments.length > 0) {
          ...
          if (docUpdateError) {
            console.error(
              "Failed to update application documents:",
              docUpdateError,
            );
          } else {
      ...
      if (profileUpdateError) {
        ...
        throw new Error(
          `Failed to update user_profiles: ${profileUpdateError.message}`,
        );
      }
      ...
      try {
        ...
        // Sempre buscar o valor líquido para garantir precisão total
        console.log(
          `[Individual Fee Payment] DEBUG - currency: ${currency}, paymentMethod: ${paymentMethod}, paymentIntentId: ${paymentIntentId}`,
        );
        ...
        // Buscar valores do Stripe para pagamentos que tenham paymentIntentId
        if (paymentIntentId) {
          console.log(
            `✅ Buscando valor líquido, bruto e taxas do Stripe (BalanceTransaction)`,
          );
          try {
            // Buscar PaymentIntent com latest_charge expandido
            ...
            if (paymentIntent.latest_charge) {
              ...
              if (charge.balance_transaction) {
                ...
                // O valor líquido (net) já está em USD se a conta for USD
                if (
                  ...
                  if (
                    ...
                    feeAmountUsd = balanceTransaction.fee / 100;
                  }

                  console.log(
                    `[Individual Fee Payment] Valores recebidos do Stripe: Líquido=${paymentAmount}, Bruto=${grossAmountUsd}, Taxas=${feeAmountUsd} (USD)`,
                  );
                } else if (
                  currency === "BRL" && session.metadata?.exchange_rate
                ) {
                  // Fallback: usar exchange_rate do metadata se não for conta USD ou falhar
                  ...
                  if (exchangeRate > 0) {
                    ...
                    console.log(
                      `[Individual Fee Payment] Usando exchange_rate do metadata (fallback): ${paymentAmount} USD`,
                    );
                  }
                }
              }
          ...
          // Fallback final para BRL sem Payment Intent
          const exchangeRate = parseFloat(session.metadata.exchange_rate);
          if (exchangeRate > 0) {
            paymentAmount = paymentAmountRaw / exchangeRate;
          }
        ...
        if (paymentIntentId) {
          ...
          if (checkError) {
            console.warn(
              "[Individual Fee Payment] Warning: Erro ao verificar duplicação:",
              checkError,
            );
          } else if (existingPayment) {
            console.log(
              `[DUPLICAÇÃO] Payment já registrado em individual_fee_payments com payment_intent_id: ${paymentIntentId}, pulando inserção.`,
            );
            duplicateFound = true;
            existingPaymentDate = existingPayment.created_at
              ? new Date(existingPayment.created_at)
              : (existingPayment.payment_date
                ? new Date(existingPayment.payment_date)
                : null);
            ...
            const { data: doubleCheckPayment, error: doubleCheckError } =
              await supabase
                .from("individual_fee_payments")
                .select("id, payment_intent_id, payment_date, created_at")
                .eq("payment_intent_id", paymentIntentId)
            ...
            if (doubleCheckError) {
              console.warn(
                "[Individual Fee Payment] Warning: Erro na segunda verificação de duplicação:",
                doubleCheckError,
              );
            } else if (doubleCheckPayment) {
              console.log(
                `[DUPLICAÇÃO] Payment já registrado (segunda verificação) com payment_intent_id: ${paymentIntentId}, pulando inserção.`,
              );
              duplicateFound = true;
              existingPaymentDate = doubleCheckPayment.created_at
                ? new Date(doubleCheckPayment.created_at)
                : (doubleCheckPayment.payment_date
                  ? new Date(doubleCheckPayment.payment_date)
                  : null);
              // Não inserir novamente
            } else {
              // Não existe, pode inserir
              console.log(
                "[Individual Fee Payment] Recording application fee payment...",
              );
              console.log(
              ...
              const { data: insertResult, error: insertError } = await supabase
                .rpc("insert_individual_fee_payment", {
                  p_user_id: userId,
              ...
              if (insertError) {
                ...
                if (
                  ...
                  console.log(
                    `[DUPLICAÇÃO] Erro de constraint única detectado, verificando se o registro foi criado por outra chamada...`,
                  );
                  ...
                  if (finalCheckPayment) {
                    console.log(
                      `[DUPLICAÇÃO] Registro foi criado por outra chamada simultânea, continuando normalmente.`,
                    );
                    duplicateFound = true;
                    existingPaymentDate = finalCheckPayment.created_at
                      ? new Date(finalCheckPayment.created_at)
                      : (finalCheckPayment.payment_date
                        ? new Date(finalCheckPayment.payment_date)
                        : null);
                  } else {
                    console.warn(
                      "[Individual Fee Payment] Warning: Erro ao inserir mas registro não encontrado:",
                      insertError,
                    );
                  }
                } else {
                ...
                console.log(
                  "[Individual Fee Payment] Application fee recorded successfully:",
                  insertResult,
                );
          ...
          console.warn(
            "[Individual Fee Payment] Warning: payment_intent_id não disponível, não é possível verificar duplicação. Pulando inserção.",
          );
        }
      } catch (recordError) {
        console.warn(
      ...
      if (finalCheckLog && finalCheckLog.length > 0) {
        ...
        if (logsWithTime.length > 0) {
          console.log(
            `[DUPLICAÇÃO] Log antigo detectado para session ${sessionId}, retornando sucesso para evitar duplicação.`,
          );
          return corsResponse({
            status: "complete",
        ...
        if (logsWithNotificationsSent.length > 0) {
          console.log(
            `[DUPLICAÇÃO] Log com notifications_sent detectado para session ${sessionId}, retornando sucesso para evitar duplicação.`,
          );
          return corsResponse({
            status: "complete",
      ...
      if (paymentInserted) {
        ...
        console.log(
          "🔔 [NOTIFICAÇÕES] Pagamento inserido com sucesso nesta execução. Enviando notificações.",
        );
        ...
        if (timeDiff > TWO_MINUTES_MS) {
          shouldSendNotifications = true;
          console.log(
            `🔔 [NOTIFICAÇÕES] Duplicata encontrada, mas registro é antigo (${timeDiff}ms > 2min). Assumindo retry e enviando notificações.`,
          );
        } else {
          shouldSendNotifications = false;
          console.log(
            `🔇 [NOTIFICAÇÕES] Duplicata encontrada e registro é recente (${timeDiff}ms < 2min). Assumindo race condition e PULANDO notificações.`,
          );
        }
      } else if (duplicateFound && !existingPaymentDate) {
        shouldSendNotifications = false;
        console.log(
          "🔇 [NOTIFICAÇÕES] Duplicata encontrada mas sem data de criação. Por segurança, PULANDO notificações.",
        );
      } else {
        ...
        shouldSendNotifications = true;
        console.log(
          "🔔 [NOTIFICAÇÕES] Caso não coberto por proteção de duplicata (ex: sem paymentIntentId). Enviando notificações por precaução.",
        );
      }
      ...
      if (shouldSendNotifications) {
        try {
          ...
          const { data: scholarship, error: scholarshipError } = await supabase
            .from("scholarships").select("id, title, university_id").eq(
              "id",
              scholarshipId,
            ).single();
          if (scholarshipError || !scholarship) {
            throw new Error("Bolsa não encontrada para notificação");
          }
          // Buscar dados da universidade
          const { data: universidade, error: univError } = await supabase.from(
            "universities",
          ).select("id, name, contact").eq("id", scholarship.university_id)
            .single();
          if (univError || !universidade) {
            throw new Error("Universidade não encontrada para notificação");
          }
          ...
          const amountValue = session.amount_total
            ? session.amount_total / 100
            : (parseFloat(session.metadata?.amount || "10"));
          ...
          const mensagemAluno =
            `O aluno ${alunoData.full_name} selecionou a bolsa "${scholarship.title}" da universidade ${universidade.name} e pagou a taxa de aplicação. Acesse o painel para revisar a candidatura.`;
          ...
          try {
            if (alunoData?.id) {
              console.log(
                "[NOTIFICAÇÃO ALUNO] Criando notificação in-app de Application Fee...",
              );
              const { error: inAppError } = await supabase
                ...
                .insert({
                  ...
                  message:
                    `Your Application Fee for ${scholarship.title} at ${universidade.name} has been confirmed.`,
          ...
          const mensagemUniversidade =
            `O aluno ${alunoData.full_name} pagou a taxa de aplicação de ${formattedAmount} via Stripe para a bolsa "${scholarship.title}" da universidade ${universidade.name}. Acesse o painel para revisar a candidatura.`;
          const universidadeNotificationPayload = {
            tipo_notf:
              "Notificação para Universidade - Pagamento de Application Fee",
            email_aluno: alunoData.email,
            nome_aluno: alunoData.full_name,
            phone_aluno: alunoData.phone || "",
            nome_bolsa: scholarship.title,
          ...
          const universidadeResult = await universidadeNotificationResponse
            .text();
          console.log(
            "[NOTIFICAÇÃO UNIVERSIDADE] Resposta do n8n (universidade):",
            universidadeNotificationResponse.status,
            universidadeResult,
          );
          // 3. NOTIFICAÇÃO PARA SELLER/ADMIN/AFFILIATE (se houver código de seller)
          console.log(
            `📤 [verify-stripe-session-application-fee] DEBUG - alunoData.seller_referral_code:`,
            alunoData.seller_referral_code,
          );
          if (alunoData.seller_referral_code) {
            console.log(
              `📤 [verify-stripe-session-application-fee] Buscando seller através do seller_referral_code: ${alunoData.seller_referral_code}`,
            );
            // Buscar informações do seller através do seller_referral_code
            console.log(
              `📤 [verify-stripe-session-application-fee] Executando query: SELECT * FROM sellers WHERE referral_code = '${alunoData.seller_referral_code}'`,
            );
            ...
            console.log(
              `📤 [verify-stripe-session-application-fee] Resultado da busca do seller:`,
              {
                sellerData,
                sellerError,
              },
            ...
            if (sellerData && !sellerError) {
              console.log(
                `📤 [verify-stripe-session-application-fee] Seller encontrado:`,
                sellerData,
              );
              ...
              if (sellerData.affiliate_admin_id) {
                console.log(
                  `📤 [verify-stripe-session-application-fee] Buscando affiliate_admin: ${sellerData.affiliate_admin_id}`,
                );
                ...
                if (affiliateData && !affiliateError) {
                  ...
                  if (affiliateProfile && !profileError) {
                    ...
                    console.log(
                      `📤 [verify-stripe-session-application-fee] Affiliate admin encontrado:`,
                      affiliateAdminData,
                    );
                  }
                }
              ...
              const sellerNotificationPayload = {
                tipo_notf:
                  "Pagamento Stripe de application fee confirmado - Seller",
                ...
                nome_universidade: universidade.name,
                o_que_enviar:
                  `Pagamento Stripe de application fee no valor de ${formattedAmount} do aluno ${alunoData.full_name} foi processado com sucesso para a bolsa "${scholarship.title}" da universidade ${universidade.name}. Seu código de referência: ${sellerData.referral_code}`,
                payment_id: sessionId,
                fee_type: "application",
                amount: amountValue,
                currency: currencyInfo.currency,
              ...
              console.log(
                "📧 [verify-stripe-session-application-fee] Enviando notificação para seller:",
              ...
              sellerNotifPromise.then(async (r) => {
                if (r.ok) {
                  console.log("[NOTIFICAÇÃO SELLER] Sucesso:", await r.text());
                } else {console.error(
                    "[NOTIFICAÇÃO SELLER] Erro:",
                    await r.text(),
                  );}
              ...
              if (sellerData.user_id) {
                try {
                  await supabase.from("admin_notifications").insert({
                    ...
                    message:
                      `Your student ${alunoData.full_name} has paid the Application Fee for "${scholarship.title}" (${formattedAmount}).`,
                    type: "payment",
              ...
              if (affiliateAdminData.email) {
                const affiliateNotificationPayload = {
                  tipo_notf:
                    "Pagamento Stripe de application fee confirmado - Affiliate Admin",
                  ...
                  nome_universidade: universidade.name,
                  o_que_enviar:
                    `Pagamento Stripe de application fee no valor de ${formattedAmount} do aluno ${alunoData.full_name} foi processado com sucesso para a bolsa "${scholarship.title}" da universidade ${universidade.name}. Seller responsável: ${sellerData.name} (${sellerData.referral_code})`,
                  payment_id: sessionId,
                  fee_type: "application",
                  amount: amountValue,
                  currency: currencyInfo.currency,
                ...
                console.log(
                  "📧 [verify-stripe-session-application-fee] Enviando notificação para affiliate admin:",
                ...
                affiliateNotifPromise.then(async (r) => {
                  if (r.ok) {
                    console.log(
                      "[NOTIFICAÇÃO AFFILIATE] Sucesso:",
                      await r.text(),
                    );
                  } else {console.error(
                      "[NOTIFICAÇÃO AFFILIATE] Erro:",
                      await r.text(),
                    );}
                }).catch((e) => console.error(e));
                ...
                if (affiliateAdminData.user_id) {
                  try {
                    await supabase.from("admin_notifications").insert({
                      ...
                      message:
                        `A student from your network (${alunoData.full_name}) has paid the Application Fee for "${scholarship.title}" (${formattedAmount}).`,
                      type: "payment",
              ...
              const adminNotificationPromises = admins.map(async (admin) => {
                ...
                console.log(
                  `📧 [verify-stripe-session-application-fee] ✅ ENVIANDO NOTIFICAÇÃO PARA ADMIN ${admin.email}:`,
                ...
                if (adminNotificationResponse.ok) {
                  ...
                  console.log(
                    `📧 [verify-stripe-session-application-fee] Notificação para ADMIN ${admin.email} enviada com sucesso:`,
                    adminResult,
                  );
                } else {
                  const adminError = await adminNotificationResponse.text();
                  console.error(
                    `📧 [verify-stripe-session-application-fee] Erro ao enviar notificação para ADMIN ${admin.email}:`,
                    adminError,
                  );
                }
                ...
                if (admin.user_id) {
                  try {
                    const { error: insertError } = await supabase.from(
                      ...
                      message:
                        `Student ${alunoData.full_name} has paid the Application Fee (${formattedAmount}).`,
                      type: "payment",
              ...
              const allPromises = [];
              if (typeof alunoNotifPromise !== "undefined") {
                allPromises.push(alunoNotifPromise);
              }
              if (typeof univNotifPromise !== "undefined") {
                allPromises.push(univNotifPromise);
              }
              if (typeof sellerNotifPromise !== "undefined") {
                allPromises.push(sellerNotifPromise);
              }
              if (typeof affiliateNotifPromise !== "undefined") {
                allPromises.push(affiliateNotifPromise);
              }
              ...
              console.log(
                `📤 [verify-stripe-session-application-fee] Seller não encontrado para seller_referral_code: ${alunoData.seller_referral_code}`,
              );
              ...
              const adminNotificationPromises = admins.map(async (admin) => {
                const adminNotificationPayload = {
                  ...
                  email_universidade: emailUniversidade,
                  o_que_enviar:
                    `Pagamento Stripe de application fee no valor de ${formattedAmount} do aluno ${alunoData.full_name} foi processado com sucesso para a bolsa "${scholarship.title}" da universidade ${universidade.name}.`,
                  payment_id: sessionId,
                  fee_type: "application",
                  amount: amountValue,
                  currency: currencyInfo.currency,
                ...
                console.log(
                  `📧 [verify-stripe-session-application-fee] ✅ ENVIANDO NOTIFICAÇÃO PARA ADMIN ${admin.email} (sem seller):`,
                  adminNotificationPayload,
                );
                const adminNotificationResponse = await fetch(
                  "https://nwh.suaiden.com/webhook/notfmatriculausa",
            ...
            console.log(
              `📤 [verify-stripe-session-application-fee] Nenhum seller_referral_code encontrado, não há seller para notificar`,
            );
            ...
            const adminNotificationPromises = admins.map(async (admin) => {
              ...
              console.log(
                `📧 [verify-stripe-session-application-fee] ✅ ENVIANDO NOTIFICAÇÃO PARA ADMIN ${admin.email} (sem seller_referral_code):`,
                adminNotificationPayload,
              );
              const adminNotificationResponse = await fetch(
                "https://nwh.suaiden.com/webhook/notfmatriculausa",
          ...
          console.error(
            "[NOTIFICAÇÃO] Erro ao notificar application fee via n8n:",
            notifErr,
          );
        }
      }
      ...
      try {
        ...
        if (userProfile) {
          await supabase.rpc("log_student_action", {
            ...
            p_action_type: "fee_payment",
            p_action_description:
              `Application Fee paid via Stripe (${sessionId}) - Notifications sent`,
            p_performed_by: userId,
            p_performed_by_type: "student",
            p_metadata: {
              fee_type: "application",
      ...
      if (paymentMethod === "pix") {
        ...
        return corsResponse({
          ...
          redirect_url:
            "http://localhost:5173/student/dashboard/application-fee-success",
        }, 200);
      }
    ...
    console.error(
      `--- CRITICAL ERROR in verify-stripe-session-application-fee ---:`,
      error.message,
    );
    return corsResponse({
