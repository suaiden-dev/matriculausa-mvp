...
Deno.serve(async (req) => {
  try {
    ...
    if (!config.clientId || !config.clientSecret) {
      console.error('[parcelow-checkout-selection-process] ❌ Credenciais Parcelow não configuradas');
      return corsResponse({ error: 'Parcelow configuration error' }, 500);
    }

    const { amount, metadata, promotional_coupon } = await req.json();
    
    console.log('[parcelow-checkout-selection-process] 📥 Payload recebido:', { amount, metadata, promotional_coupon });
    
    const authHeader = req.headers.get('Authorization');
    if (!authHeader) {
      console.error('[parcelow-checkout-selection-process] ❌ Header de autorização não encontrado');
    ...
    if (authError || !user) {
      console.error('[parcelow-checkout-selection-process] ❌ Erro de autenticação:', authError);
      return corsResponse({ error: 'Invalid token' }, 401);
    }

    console.log('[parcelow-checkout-selection-process] ✅ Usuário autenticado:', user.id);
    ...
    if (promotional_coupon && promotional_coupon.trim()) {
      try {
        ...
        console.log('[parcelow-checkout-selection-process] 🎟️ Validando cupom promocional:', normalizedCoupon);
        ...
        if (couponError) {
          console.error('[parcelow-checkout-selection-process] ❌ Erro ao validar cupom promocional:', couponError);
          ...
          console.log('[parcelow-checkout-selection-process] ✅ Cupom promocional válido! Novo valor:', finalAmount);
        } else {
          console.log('[parcelow-checkout-selection-process] ⚠️ Cupom promocional inválido');
        }
      } catch (error) {
        console.error('[parcelow-checkout-selection-process] ❌ Erro ao verificar cupom promocional:', error);
      }
    }
    ...
    if (profileError || !profile) {
      console.error('[parcelow-checkout-selection-process] ❌ Erro ao buscar perfil:', profileError);
      return corsResponse({ error: 'User profile not found' }, 404);
    }

    if (!profile.cpf_document) {
      console.error('[parcelow-checkout-selection-process] ❌ CPF é obrigatório para pagamento via Parcelow');
    ...
    const redirectSuccess = `${origin}/student/dashboard/selection-process-fee-success?reference=${encodeURIComponent(reference)}`;
    const redirectFailed = `${origin}/student/dashboard/selection-process-fee-error?reference=${encodeURIComponent(reference)}`;
    ...
    console.log('[parcelow-checkout-selection-process] 🛒 Criando pedido na Parcelow...');
    console.log('[parcelow-checkout-selection-process] 🔍 Config:', {
      apiBaseUrl: config.apiBaseUrl,
      environment: config.environment,
      hasToken: !!accessToken
    });
    console.log('[parcelow-checkout-selection-process] 🔍 Order Payload:', JSON.stringify(orderPayload, null, 2));
    ...
    console.log('[parcelow-checkout-selection-process] 📡 Response Status:', orderResponse.status);
    console.log('[parcelow-checkout-selection-process] 📡 Response Headers:', Object.fromEntries(orderResponse.headers.entries()));

    // Obter o texto da resposta primeiro para debug
    const responseText = await orderResponse.text();
    console.log('[parcelow-checkout-selection-process] 📡 Response Body (first 500 chars):', responseText.substring(0, 500));

    if (!orderResponse.ok) {
      console.error('[parcelow-checkout-selection-process] ❌ Erro ao criar pedido na Parcelow:', responseText);
      return corsResponse({ error: 'Failed to create Parcelow order', details: responseText }, 500);
    }
    ...
    try {
      ...
      console.error('[parcelow-checkout-selection-process] ❌ Erro ao fazer parse do JSON da resposta:', parseError);
      console.error('[parcelow-checkout-selection-process] ❌ Response text:', responseText);
      return corsResponse({ 
        error: 'Invalid JSON response from Parcelow', 
        details: 'Received HTML instead of JSON. The API endpoint might be incorrect or the service is down.',
        responsePreview: responseText.substring(0, 200)
      }, 500);
    }
    console.log('[parcelow-checkout-selection-process] ✅ Pedido criado na Parcelow');
    console.log('[parcelow-checkout-selection-process] 🔍 Parcelow Order Response:', JSON.stringify(parcelowOrder, null, 2));
    ...
    if (!orderId || !checkoutUrl) {
      console.error('[parcelow-checkout-selection-process] ❌ Resposta da Parcelow não contém order_id ou checkout_url:', parcelowOrder);
      return corsResponse({ 
    ...
    console.log('[parcelow-checkout-selection-process] ✅ Checkout URL:', checkoutUrl);
    ...
    if (insertError) {
      console.error('[parcelow-checkout-selection-process] ❌ Erro ao registrar pagamento:', insertError);
    } else {
      console.log('[parcelow-checkout-selection-process] ✅ Pagamento registrado com sucesso!');
    }
    ...
    try {
      await supabase.rpc('log_student_action', {
        ...
        p_action_description: `Parcelow checkout session created for selection_process (${orderId})`,
    ...
    console.error('[parcelow-checkout-selection-process] ❌ Erro geral na função:', error);
