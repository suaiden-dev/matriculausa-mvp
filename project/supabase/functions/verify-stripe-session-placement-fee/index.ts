...
Deno.serve(async (req) => {
  console.log('--- verify-stripe-session-placement-fee: Request received ---');
  try {
    ...
    await supabase.rpc('log_student_action', {
      ...
      p_metadata: { fee_type: 'placement', payment_method: 'stripe', amount: amountPaid, session_id: sessionId }
    });
    ...
    const alunoPayload = {
      tipo_notf: 'Pagamento Stripe de Placement Fee confirmado',
    ...
    try {
      ...
      if (userProfile.seller_referral_code) {
        ...
        if (sellerData) {
          ...
          const sellerPayload = {
            tipo_notf: 'Pagamento de Placement Fee confirmado - Seller',
            email_seller: sellerData.email,
            nome_seller: sellerData.name,
            phone_seller: sellerProfile?.phone || "",
            email_aluno: userProfile.email,
            nome_aluno: userProfile.full_name,
            o_que_enviar: `Parabéns! Seu aluno ${userProfile.full_name} pagou a Placement Fee no valor de ${formattedAmount} via Stripe. Sua comissão será calculada em breve.`,
            payment_id: sessionId,
            fee_type: 'placement',
            amount: amountPaid,
          ...
          if (affiliateAdminData) {
            const affiliatePayload = {
              tipo_notf: 'Pagamento de Placement Fee confirmado - Affiliate Admin',
              ...
              email_seller: sellerData.email,
              nome_seller: sellerData.name,
              o_que_enviar: `O aluno ${userProfile.full_name} do seu seller ${sellerData.name} pagou a Placement Fee no valor de ${formattedAmount} via Stripe.`,
              payment_id: sessionId,
              fee_type: 'placement',
              amount: amountPaid,
              seller_id: sellerData.user_id,
      ...
      for (const admin of admins) {
        const adminPayload = {
          tipo_notf: 'Pagamento de Placement Fee confirmado - Admin',
          email_admin: admin.email,
          nome_admin: admin.full_name,
          phone_admin: admin.phone || "",
          email_aluno: userProfile.email,
          nome_aluno: userProfile.full_name,
          o_que_enviar: `Pagamento Stripe de Placement Fee no valor de ${formattedAmount} do aluno ${userProfile.full_name} foi confirmado.`,
          payment_id: sessionId,
          fee_type: 'placement',
          amount: amountPaid,
          payment_method: 'stripe'
    ...
    return corsResponse({ status: 'complete', message: 'Placement fee processed successfully.' }, 200);
  } catch (error) {
    console.error('Error in placement-fee function:', error);
