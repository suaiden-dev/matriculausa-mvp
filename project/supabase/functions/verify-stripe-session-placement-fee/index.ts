...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!userId) {
      return corsResponse({ error: "User ID missing in session." }, 400);
    }
    ...
    try {
      ...
      if (userProfile.seller_referral_code) {
        ...
        if (sellerData) {
          ...
          const sellerPayload = {
            ...
            nome_aluno: userProfile.full_name,
            o_que_enviar:
              `Parabéns! Seu aluno ${userProfile.full_name} pagou a Placement Fee no valor de ${formattedAmount} via Stripe. Sua comissão será calculada em breve.`,
            payment_id: sessionId,
            fee_type: "placement",
            amount: amountPaid,
          ...
          if (affiliateAdminData) {
            const affiliatePayload = {
              ...
              nome_seller: sellerData.name,
              o_que_enviar:
                `O aluno ${userProfile.full_name} do seu seller ${sellerData.name} pagou a Placement Fee no valor de ${formattedAmount} via Stripe.`,
              payment_id: sessionId,
              fee_type: "placement",
              amount: amountPaid,
              seller_id: sellerData.user_id,
            ...
            fetch("https://nwh.suaiden.com/webhook/notfmatriculausa", {
              ...
              console.error("Error sending affiliate notification:", e)
            );
          }
        }
      }
      ...
      for (const admin of admins) {
        const adminPayload = {
          ...
          nome_aluno: userProfile.full_name,
          o_que_enviar:
            `Pagamento Stripe de Placement Fee no valor de ${formattedAmount} do aluno ${userProfile.full_name} foi confirmado.`,
          payment_id: sessionId,
          fee_type: "placement",
          amount: amountPaid,
          payment_method: "stripe",
        ...
        fetch("https://nwh.suaiden.com/webhook/notfmatriculausa", {
          ...
          console.error(
            `Error sending admin notification to ${admin.email}:`,
            e,
          )
        );
      }
    ...
    return corsResponse({
      status: "complete",
      message: "Placement fee processed successfully.",
    }, 200);
  } catch (error) {
