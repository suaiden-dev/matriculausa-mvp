...
Deno.serve(async (req) => {
  try {
    ...
    const { amount, metadata, scholarships_ids, promotional_coupon, cpf: bodyCpf, payer_info } = await req
      .json();
      ...
      client: {
        ...
        email: (() => {
          ...
          if (isSandbox && email) {
            ...
            console.log(`[parcelow-checkout-scholarship-fee] 🧪 Sandbox detected. Using dirty email bypass: ${dirtyEmail}`);
            return dirtyEmail;
          }
          return email;
        })(),
    ...
    if (parcelowOrder.success === false) {
      console.error(
        "[parcelow-checkout-scholarship-fee] ❌ Parcelow retornou erro de negócio:",
