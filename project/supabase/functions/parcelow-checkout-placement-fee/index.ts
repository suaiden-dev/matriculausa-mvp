...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!finalCpf || finalCpf.length < 11) {
      console.error(
        "[parcelow-checkout-placement-fee] ❌ CPF não encontrado no perfil, body ou payer_info",
      );
      return corsResponse({
      ...
      client: {
        ...
        email: (() => {
          ...
          if (isSandbox && email) {
            ...
            console.log(`[parcelow-checkout-placement-fee] 🧪 Sandbox detected. Using dirty email bypass: ${dirtyEmail}`);
            return dirtyEmail;
          }
          return email;
        })(),
    ...
    if (parcelowOrder.success === false) {
      console.error(
        "[parcelow-checkout-placement-fee] ❌ Parcelow retornou erro de negócio:",
    ...
    }).catch((err) => console.warn("[parcelow-checkout-placement-fee] Notifier error (ignorado):", err));
    // ==========================================
