...
Deno.serve(async (req) => {
  try {
    ...
    if (!config.clientId || !config.clientSecret) {
      console.error('[parcelow-checkout-application-fee] ❌ Credenciais Parcelow não configuradas');
      return corsResponse({ error: 'Parcelow configuration error' }, 500);
    }

    const { metadata } = await req.json();
    
    console.log('[parcelow-checkout-application-fee] 📥 Payload recebido:', { metadata });
    
    const authHeader = req.headers.get('Authorization');
    if (!authHeader) {
      console.error('[parcelow-checkout-application-fee] ❌ Header de autorização não encontrado');
    ...
    if (authError || !user) {
      console.error('[parcelow-checkout-application-fee] ❌ Erro de autenticação:', authError);
      return corsResponse({ error: 'Invalid token' }, 401);
    }

    console.log('[parcelow-checkout-application-fee] ✅ Usuário autenticado:', user.id);

    // Buscar perfil do usuário para obter CPF e dependentes
    const { data: userProfile, error: profileError } = await supabase
      .from('user_profiles')
      .select('id, user_id, full_name, email, cpf_document, phone, system_type, dependents')
      .eq('user_id', user.id)
      .single();

    if (profileError || !userProfile) {
      console.error('[parcelow-checkout-application-fee] ❌ Erro ao buscar perfil:', profileError);
      return corsResponse({ error: 'User profile not found' }, 404);
    }

    if (!userProfile.cpf_document) {
      console.error('[parcelow-checkout-application-fee] ❌ CPF é obrigatório para pagamento via Parcelow');
    ...
    if (appError || !application) {
      console.log('[parcelow-checkout-application-fee] Application not found, attempting to create new one');
      
      const scholarshipId = metadata?.selected_scholarship_id || metadata?.scholarship_id;
      if (!scholarshipId) {
        console.error('[parcelow-checkout-application-fee] No scholarship_id in metadata to create application');
      ...
      if (insertError || !newApp) {
        console.error('[parcelow-checkout-application-fee] Error creating application:', insertError);
        return corsResponse({ error: 'Failed to create application' }, 500);
      }

      application = newApp;
      applicationId = newApp.id;
      console.log('[parcelow-checkout-application-fee] New application created:', application.id);
    } else {
      console.log('[parcelow-checkout-application-fee] Application verified:', application.id);
    ...
    if (finalAmountFromMetadata) {
      ...
      console.log('[parcelow-checkout-application-fee] ✅ Usando valor com desconto do metadata:', applicationFeeAmount);
    } else {
      ...
      if (application.scholarship_id) {
        ...
        if (!scholarshipError && scholarshipData) {
          ...
          console.log('[parcelow-checkout-application-fee] Valor da bolsa:', applicationFeeAmount);
        }
      }
      ...
      if (dependents > 0) {
        ...
        console.log(`[parcelow-checkout-application-fee] ✅ Adicionado $${dependentsCost} por ${dependents} dependente(s). Novo valor: $${applicationFeeAmount}`);
      }
    }
    ...
    console.log('[parcelow-checkout-application-fee] 🛒 Criando pedido na Parcelow...');
    console.log('[parcelow-checkout-application-fee] 🔍 Order Payload:', JSON.stringify(orderPayload, null, 2));
    ...
    if (!orderResponse.ok) {
      ...
      console.error('[parcelow-checkout-application-fee] ❌ Erro ao criar pedido na Parcelow:', error);
      return corsResponse({ error: 'Failed to create Parcelow order', details: error }, 500);
    }

    const parcelowOrder = await orderResponse.json();
    console.log('[parcelow-checkout-application-fee] ✅ Pedido criado na Parcelow');
    console.log('[parcelow-checkout-application-fee] 🔍 Parcelow Order Response:', JSON.stringify(parcelowOrder, null, 2));
    ...
    if (!orderId || !checkoutUrl) {
      console.error('[parcelow-checkout-application-fee] ❌ Resposta da Parcelow não contém order_id ou checkout_url:', parcelowOrder);
      return corsResponse({ 
    ...
    console.log('[parcelow-checkout-application-fee] ✅ Checkout URL:', checkoutUrl);
    ...
    if (insertError) {
      console.error('[parcelow-checkout-application-fee] ❌ Erro ao registrar pagamento:', insertError);
    } else {
      console.log('[parcelow-checkout-application-fee] ✅ Pagamento registrado com sucesso!');
    }
    ...
    try {
      await supabase.rpc('log_student_action', {
        ...
        p_action_description: `Parcelow checkout session created for application_fee (${orderId})`,
    ...
    console.error('[parcelow-checkout-application-fee] ❌ Erro geral na função:', error);
