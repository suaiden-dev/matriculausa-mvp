...
Deno.serve(async (req) => {
  try {
      ...
      client: {
        ...
        email: (() => {
          ...
          if (isSandbox && email) {
            ...
            console.log(`[parcelow-checkout-i20-control-fee] 🧪 Sandbox detected. Using dirty email bypass: ${dirtyEmail}`);
            return dirtyEmail;
          }
          return email;
        })(),
    ...
    if (parcelowOrder.success === false) {
      console.error(
        "[parcelow-checkout-i20-control-fee] ❌ Parcelow retornou erro de negócio:",
