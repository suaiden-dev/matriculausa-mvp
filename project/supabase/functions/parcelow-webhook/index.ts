...
> {
  try {
    ...
    if (admins.length === 0) {
      console.warn(
        "[getAllAdmins] Nenhum admin válido após filtro, usando admin padrão",
      );
      return [{
        user_id: "",
        email: "admin@matriculausa.com",
...
async function sendTermAcceptanceNotificationAfterPayment(
  ...
  try {
    ...
    if (!finalPhotoPath || finalPhotoPath === "mock_localhost_photo.png") {
      console.log(
        "[NOTIFICAÇÃO] Foto no aceite atual é nula ou mock. Buscando foto real em outros aceites...",
      );
      ...
      if (altTerm) {
        ...
        console.log(
          "[NOTIFICAÇÃO] ✅ Foto real encontrada em outro aceite:",
          finalPhotoPath,
        );
      }
    }
    ...
    try {
      ...
      if (termData.content && termData.content.trim() !== "") {
        ...
        try {
          ...
          const fallbackText = termData.content.replace(/<[^>]*>/g, "")
            .substring(0, 3000);
          currentY = addWrappedText(
            fallbackText,
            margin,
      ...
      if (finalPhotoPath && finalPhotoPath.trim() !== "") {
        try {
          console.log(
            "[NOTIFICAÇÃO] Foto de identidade para inclusão no PDF:",
            finalPhotoPath,
          );
          ...
          if (finalPhotoPath === "mock_localhost_photo.png") {
            ...
            if (!imageError && imageData) {
              try {
                ...
                const fileExtension =
                  finalPhotoPath.split(".").pop()?.toLowerCase() || "jpg";
                ...
                try {
                  ...
                  console.warn(
                    "[NOTIFICAÇÃO] Erro ao obter props da imagem, usando fallback:",
                    pErr,
                  );
                ...
                console.log(
                  "[NOTIFICAÇÃO] ✅ Foto de identidade incluída no PDF!",
                );
              } catch (convErr) {
                console.error(
                  "[NOTIFICAÇÃO] Erro na conversão/processamento da imagem:",
                  convErr,
                );
              }
            } else {
              console.warn(
                "[NOTIFICAÇÃO] Falha ao baixar foto real do storage:",
                imageError?.message,
              );
            }
          }
          ...
          console.error(
            "[NOTIFICAÇÃO] Erro geral ao processar foto:",
            photoErr,
          );
        }
      }
    ...
    if (isDevelopment) {
      if (devBlockedEmails.includes(emailAluno)) {
        console.log(
          `[NOTIFICAÇÃO] Email de aluno bloqueado em desenvolvimento: ${emailAluno}`,
        );
        emailAluno = "";
      }
      if (
        emailSellerAcceptance &&
        devBlockedEmails.includes(emailSellerAcceptance)
      ) {
        console.log(
          `[NOTIFICAÇÃO] Email de seller bloqueado em desenvolvimento: ${emailSellerAcceptance}`,
        );
        emailSellerAcceptance = "";
      }
      if (
        emailAffiliateAdminAcceptance &&
        devBlockedEmails.includes(emailAffiliateAdminAcceptance)
      ) {
        console.log(
          `[NOTIFICAÇÃO] Email de affiliate admin bloqueado em desenvolvimento: ${emailAffiliateAdminAcceptance}`,
        );
        emailAffiliateAdminAcceptance = "";
      }
    }
    ...
    const webhookPayload = {
      ...
      o_que_enviar:
        `Student ${userProfile.full_name} has accepted the Student Checkout Terms & Conditions and completed ${feeType} payment via Parcelow. This shows the student is progressing through the enrollment process.`,
    ...
    if (webhookResponse.ok) {
      console.log(
        `[NOTIFICAÇÃO] Notificação de aceitação de termos enviada com sucesso para ${userProfile.email}!`,
      );
    } else {
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (reference.startsWith("APK_")) {
      console.log(
        `[router] Detectada ordem da Aplikei: ${reference}. Redirecionando...`,
      );

      const APLIKEI_WEBHOOK_URL =
        "https://nkhblkilekfpqhyuhrrj.supabase.co/functions/v1/parcelow-webhook";
      ...
      try {
        ...
        console.log(
          `[router] Resposta da Aplikei (Status: ${response.status}):`,
          resultText,
        );
        ...
        console.error(
          "[router] ❌ Falha crítica de conexão ao repassar webhook para Aplikei:",
          err,
        );
        return new Response(
    ...
    if (!userId && reference) {
      ...
      if (paymentByRef) {
        ...
        userId = payment.user_id;
        if (
          !payment.parcelow_order_id ||
          payment.parcelow_order_id !== String(parcelowOrder.id)
        ) {
          await supabase
            .from("individual_fee_payments")
    ...
    const netAmount = (parcelowOrder.order_amount || parcelowOrder.net_amount ||
        (parcelowOrder.items?.[0]?.amount))
      ? ((parcelowOrder.order_amount || parcelowOrder.net_amount ||
        parcelowOrder.items?.[0]?.amount) / 100)
      : (payment?.amount || 0);
    ...
    const rawTotalUsd = parcelowOrder.total_usd || parcelowOrder.totalUsd ||
      parcelowOrder.total_amount_usd || 0;
    ...
    const feeAmountUSD = Number(
      Math.max(0, grossAmount - netAmount).toFixed(2),
    );
    ...
    switch (event) {
      ...
      case "event_order_confirmed":
        newStatus = "confirmed";
        console.log(
          "[parcelow-webhook] ✅ Pedido confirmado, aguardando pagamento",
        );
        break;
    ...
    console.log(
      `[parcelow-webhook] 🔄 Status mapeado: ${newStatus} (isPaid: ${isPaid})`,
    );
    ...
    if (payment?.fee_type) {
      ...
      console.log(
        "[parcelow-webhook] 📋 Fee type recuperado do banco de dados:",
        feeType,
      );
    } else {
      // Tentar detectar pelo prefixo da reference
      if (
        reference.startsWith("app_fee_") ||
        reference.startsWith("applicatio_") || reference.startsWith("app_") ||
        reference.startsWith("ap_")
      ) {
        feeType = "application_fee";
      } else if (
        reference.startsWith("sp_") || reference.startsWith("selection_")
      ) {
        feeType = "selection_process";
      } else if (
        reference.startsWith("sf_") || reference.startsWith("scholarship_") ||
    ...
    console.log(
      "[parcelow-webhook] 📋 Fee type detectado inicialmente:",
      feeType,
    );
    ...
    if (payment) {
      console.log(
        `[parcelow-webhook] 🔄 Atualizando payment record: ${payment.parcelow_status} -> ${newStatus}`,
      );
      ...
      if (updateError) {
        console.error(
          "[parcelow-webhook] ❌ Erro ao atualizar pagamento:",
          updateError,
        );
        throw updateError;
      }
      ...
      console.log(
        "[parcelow-webhook] 📝 Criando payment record (não existia)...",
      );
      ...
      if (insertError) {
        console.error(
          "[parcelow-webhook] ❌ Erro ao criar payment record:",
          insertError,
        );
      } else {
      ...
      console.log(
        "[parcelow-webhook] ⚠️ Payment record não existe e pagamento não foi confirmado ainda",
      );
    }

    // 4.0. Detectar ambiente (dev ou prod) - verificar redirect_success URL
    const redirectUrl = parcelowOrder.redirect_success || "";
    const isDevelopment = redirectUrl.includes("localhost") ||
      redirectUrl.includes("127.0.0.1") ||
      redirectUrl.includes("staging-matriculausa.netlify.app");
    console.log(
      "[parcelow-webhook] 🌍 Ambiente detectado:",
      isDevelopment ? "DESENVOLVIMENTO/STAGING" : "PRODUÇÃO",
    );

    // 5. Se pago, processar lógica completa (igual ao Stripe)
    if (isPaid) {
      console.log(
        "[parcelow-webhook] ✅ Pagamento CONFIRMADO! Processando lógica completa...",
      );
      ...
      if (existingLogs && existingLogs.length > 0) {
        ...
        if (hasNotificationLog) {
          console.log(
            `[parcelow-webhook] ⚠️ Pagamento já processado anteriormente, ignorando duplicação`,
          );
          return new Response(
      ...
      if (profileFetchError || !userProfile) {
        console.error(
          "[parcelow-webhook] ❌ Erro ao buscar perfil:",
          profileFetchError,
        );
        return new Response(
      ...
      try {
        await supabase.rpc("log_student_action", {
          ...
          p_action_description:
            `${feeType} payment processing started (Parcelow ${parcelowOrder.id})`,
        ...
        console.log(
          "[parcelow-webhook] ✅ Log de início de processamento criado",
        );
      } catch (logError) {
      ...
      switch (feeType) {
        case "selection_process":
          ...
          if (profileUpdateError) {
            console.error(
              "[parcelow-webhook] ❌ Erro ao atualizar perfil:",
              profileUpdateError,
            );
          } else {
            console.log(
              "[parcelow-webhook] ✅ has_paid_selection_process_fee atualizado!",
            );
          }
          break;
        ...
        case "application_fee":
          ...
          const { data: application, error: appFetchError } = await supabase
            ...
            .select(
              "id, student_id, scholarship_id, student_process_type, status",
            )
          ...
          if (appFetchError || !application) {
            console.error(
              "[parcelow-webhook] ❌ Nenhuma aplicação não paga encontrada para o usuário:",
              userId,
            );
          ...
          console.log(
            "[parcelow-webhook] ✅ Aplicação encontrada:",
            application.id,
          );
          ...
          if (application.status !== "approved") {
            ...
            console.log(
              "[parcelow-webhook] ✅ Application status set to under_review",
            );
          } else {
          ...
          if (appUpdateError) {
            console.error(
              "[parcelow-webhook] ❌ Erro ao atualizar application:",
              appUpdateError,
            );
          } else {
            console.log(
              "[parcelow-webhook] ✅ Application atualizada com sucesso",
            );
          }
        ...
        case "scholarship_fee":
          ...
          let scholarshipsIds = scholarshipMetadata.scholarships_ids ||
            scholarshipMetadata.scholarship_id;

          if (!scholarshipsIds) {
            console.log(
              "[parcelow-webhook] 🔍 Metadata scholarships_ids não encontrado, buscando aplicação mais recente não paga...",
            );
            ...
            if (latestApp) {
              console.log(
                "[parcelow-webhook] ✅ Aplicação encontrada via busca:",
                latestApp.id,
              );
          ...
          if (scholarshipsIds) {
            ...
            for (const scholarshipId of idsArray) {
              ...
              if (scholarshipUpdateError) {
                console.error(
                  `[parcelow-webhook] ❌ Erro ao atualizar scholarship ${scholarshipId}:`,
                  scholarshipUpdateError,
                );
              } else {
                console.log(
                  `[parcelow-webhook] ✅ Scholarship ${scholarshipId} atualizada`,
                );
            ...
            console.error(
              "[parcelow-webhook] ❌ Nenhum ID de bolsa encontrado para processar",
            );
          }
        ...
        case "i20_control_fee":
          ...
          if (i20UpdateError) {
            console.error(
              "[parcelow-webhook] ❌ Erro ao atualizar i20 control:",
              i20UpdateError,
            );
          } else {
            console.log(
              "[parcelow-webhook] ✅ I-20 control fee status atualizado!",
            );
          }
          break;
      ...
      if (feeType === "i20_control" || feeType === "i20_control_fee") {
        try {
          ...
          if (!usedError && usedCode && usedCode.referrer_id) {
            ...
            console.log(
              "[parcelow-webhook] 🎁 Enviando notificação de recompensa para o padrinho...",
            );
            ...
            if (referrerProfile?.email) {
              ...
              if (
                isDevelopment && devBlockedEmails.includes(emailReferrerFinal)
              ) {
                console.log(
                  "[parcelow-webhook] 🚫 Email de padrinho bloqueado em desenvolvimento:",
                  emailReferrerFinal,
                );
                emailReferrerFinal = "";
              }
              ...
              if (emailReferrerFinal) {
                ...
                console.log(
                  "✅ [parcelow-webhook] Notificação de recompensa enviada!",
                );
              }
            }
          }
          ...
          console.error(
            "[parcelow-webhook] ❌ Erro ao processar recompensas de referência:",
            rewardError,
          );
        }
      } else {
        console.log(
          `[parcelow-webhook] ℹ️ Recompensas ignoradas para fee_type: ${feeType} (recompensas são vinculadas apenas ao I20)`,
        );
      }
      ...
      try {
        ...
        if (cartError) {
          console.error(
            "[parcelow-webhook] ❌ Erro ao limpar carrinho:",
            cartError,
          );
        } else {
      ...
      try {
        await supabase.rpc("log_student_action", {
          ...
          p_action_type: "fee_payment",
          p_action_description:
            `${feeType} notifications sending started (Parcelow ${parcelowOrder.id})`,
          p_performed_by: userId,
          p_performed_by_type: "student",
          p_metadata: {
            fee_type: feeType,
        ...
        console.log(
          "[parcelow-webhook] ✅ Log de envio de notificações criado",
        );
      } catch (logError) {
        console.error(
          "[parcelow-webhook] ❌ Erro ao criar log de notificações:",
          logError,
        );
      }
      ...
      try {
        console.log(
          "[parcelow-webhook] 📄 Enviando notificação de aceitação de termos...",
        );
        ...
        console.error(
          "[parcelow-webhook] ❌ Erro ao enviar notificação de termos:",
          termNotifError,
        );
      }
      ...
      try {
        ...
        if (isDevelopment && devBlockedEmails.includes(emailAlunoFinal)) {
          console.log(
            "[parcelow-webhook] 🚫 Email de aluno bloqueado em desenvolvimento:",
            emailAlunoFinal,
          );
          emailAlunoFinal = "";
        }
        ...
        console.log(
          "[parcelow-webhook] 📧 Enviando notificação para aluno:",
          userProfile.email,
        );
        ...
        if (alunoNotificationResponse.ok) {
          console.log(
            "[parcelow-webhook] ✅ Notificação para aluno enviada com sucesso!",
          );
        } else {
          const errorText = await alunoNotificationResponse.text();
          console.error(
            "[parcelow-webhook] ❌ Erro ao enviar notificação para aluno:",
            errorText,
          );
        }
        ...
        try {
          const { error: inAppError } = await supabase
            ...
            .insert({
              ...
              message: `Your ${
                feeType.replace("_", " ")
              } has been confirmed. You can now proceed to select your schools.`,
          ...
          if (inAppError) {
            console.error(
              "[parcelow-webhook] ❌ Erro ao criar notificação in-app para aluno:",
              inAppError,
            );
          } else {
            console.log(
              "[parcelow-webhook] ✅ Notificação in-app criada para aluno!",
            );
          }
        } catch (inAppEx) {
          console.error(
            "[parcelow-webhook] ❌ Exceção ao criar notificação in-app:",
            inAppEx,
          );
        }
        ...
        if (userProfile.seller_referral_code) {
          console.log(
            "[parcelow-webhook] 🔍 Buscando seller:",
          ...
          if (sellerData && !sellerError) {
            console.log(
              "[parcelow-webhook] ✅ Seller encontrado:",
              sellerData.name,
            );
            ...
            if (sellerData.affiliate_admin_id) {
              ...
              if (affiliateData) {
                ...
                if (affiliateProfile) {
                  ...
                  console.log(
                    "[parcelow-webhook] ✅ Affiliate admin encontrado:",
                    affiliateAdminData.name,
                  );
                }
              }
            }
            ...
            const adminNotificationPromises = admins.map(async (admin) => {
              ...
              const adminNotificationPayload = {
                ...
                o_que_enviar:
                  `Pagamento Parcelow de ${feeType} no valor de ${formattedAmount} do aluno ${userProfile.full_name} foi processado com sucesso.${
                    ...
                    affiliateAdminData.user_id
                      ? ` Affiliate: ${affiliateAdminData.name}`
                      : ""
                  }`,
                payment_id: String(parcelowOrder.id),
                fee_type: feeType,
              ...
              if (adminResponse.ok) {
                console.log(
                  `[parcelow-webhook] ✅ Notificação enviada para admin ${admin.email}`,
                );
              } else {
                const errorText = await adminResponse.text();
                console.error(
                  `[parcelow-webhook] ❌ Erro ao notificar admin ${admin.email}:`,
                  errorText,
                );
              }
              ...
              if (admin.user_id) {
                try {
                  await supabase.from("admin_notifications").insert({
                    ...
                    message: `Student ${userProfile.full_name} has paid the ${
                      feeType.replace("_", " ")
                    } (${formattedAmount}) via Parcelow.`,
                    type: "payment",
                  ...
                  console.log(
                    `[parcelow-webhook] ✅ In-app notification criada para admin ${admin.email}`,
                  );
                } catch (adminInAppErr) {
                  console.error(
                    `[parcelow-webhook] ❌ Erro ao criar in-app para admin:`,
                    adminInAppErr,
                  );
                }
              }
            ...
            if (feeType === "scholarship_fee") {
              tipoNotfSeller =
                "Pagamento Stripe de scholarship fee confirmado - Seller";
            } else if (
              feeType === "i20_control" || feeType === "i20_control_fee"
            ) {
              tipoNotfSeller =
                "Pagamento Stripe de I-20 control fee confirmado - Seller";
              oQueEnviarSeller =
                `Parabéns! Seu aluno ${userProfile.full_name} pagou a taxa I-20 Control Fee no valor de ${formattedAmount} via Parcelow. Sua comissão será calculada em breve.`;
            } else if (
              feeType === "application_fee" || feeType === "application"
            ) {
              tipoNotfSeller =
            ...
            if (isDevelopment && devBlockedEmails.includes(emailSellerFinal)) {
              console.log(
                "[parcelow-webhook] 🚫 Email de seller bloqueado em desenvolvimento:",
                emailSellerFinal,
              );
              emailSellerFinal = "";
            }
            ...
            if (sellerResponse.ok) {
              console.log(
                "[parcelow-webhook] ✅ Notificação enviada para seller!",
              );
            } else {
              const errorText = await sellerResponse.text();
              console.error(
                "[parcelow-webhook] ❌ Erro ao notificar seller:",
                errorText,
              );
            }
            ...
            if (sellerData.user_id) {
              try {
                await supabase.from("admin_notifications").insert({
                  ...
                  message:
                    `Your student ${userProfile.full_name} has paid the ${
                      feeType.replace("_", " ")
                    } (${formattedAmount}) via Parcelow.`,
                  type: "payment",
                ...
                console.log(
                  "[parcelow-webhook] ✅ In-app notification criada para seller!",
                );
              } catch (sellerInAppErr) {
                console.error(
                  "[parcelow-webhook] ❌ Erro ao criar in-app para seller:",
                  sellerInAppErr,
                );
              }
            }
            ...
            if (affiliateAdminData.email) {
              ...
              if (feeType === "scholarship_fee") {
                tipoNotfAffiliate =
                  "Pagamento Stripe de scholarship fee confirmado - Affiliate Admin";
              } else if (
                feeType === "i20_control" || feeType === "i20_control_fee"
              ) {
                tipoNotfAffiliate =
                  "Pagamento Stripe de I-20 control fee confirmado - Affiliate Admin";
              } else if (
                feeType === "application_fee" || feeType === "application" ||
                feeType === "application_fee_payment"
              ) {
              ...
              if (
                isDevelopment && devBlockedEmails.includes(emailAffiliateFinal)
              ) {
                console.log(
                  "[parcelow-webhook] 🚫 Email de affiliate admin bloqueado em desenvolvimento:",
                  emailAffiliateFinal,
                );
                emailAffiliateFinal = "";
              }
              ...
              if (affiliateResponse.ok) {
                console.log(
                  "[parcelow-webhook] ✅ Notificação enviada para affiliate admin!",
                );
              } else {
                const errorText = await affiliateResponse.text();
                console.error(
                  "[parcelow-webhook] ❌ Erro ao notificar affiliate admin:",
                  errorText,
                );
              }
              ...
              if (affiliateAdminData.user_id) {
                try {
                  await supabase.from("admin_notifications").insert({
                    ...
                    message:
                      `A student from your network (${userProfile.full_name}) has paid the ${
                        feeType.replace("_", " ")
                      } (${formattedAmount}) via Parcelow.`,
                    type: "payment",
                  ...
                  console.log(
                    "[parcelow-webhook] ✅ In-app notification criada para affiliate admin!",
                  );
                } catch (affiliateInAppErr) {
                  console.error(
                    "[parcelow-webhook] ❌ Erro ao criar in-app para affiliate:",
                    affiliateInAppErr,
                  );
                }
              }
            ...
            console.log(
              "[parcelow-webhook] ⚠️ Seller não encontrado, notificando apenas admins",
            );
          ...
          console.log(
            "[parcelow-webhook] ⚠️ Sem seller_referral_code, notificando apenas admins",
          );
          ...
          const adminNotificationPromises = admins.map(async (admin) => {
            ...
            let tipoNotfAdminNoReferral =
              `Pagamento Stripe de selection process confirmado - Admin`; // Default
        ...
        console.error(
          "[parcelow-webhook] ❌ Erro ao processar notificações:",
          notifError,
        );
      }
      ...
      try {
        await supabase.rpc("log_student_action", {
          ...
          p_action_type: "fee_payment",
          p_action_description:
            `${feeType} paid via Parcelow (${parcelowOrder.id}) - Notifications sent`,
          p_performed_by: userId,
          p_performed_by_type: "student",
          p_metadata: {
            fee_type: feeType,
        ...
        console.error(
          "[parcelow-webhook] ❌ Erro ao criar log de conclusão:",
          logError,
        );
      }

      console.log(
        "[parcelow-webhook] 🎉 Processamento completo finalizado com sucesso!",
      );
    }
