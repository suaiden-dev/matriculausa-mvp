...
export function getStripeConfig(req: Request): StripeConfig {
  ...
  if (validationErrors.length > 0) {
    throw new Error(
      `Stripe configuration errors: ${validationErrors.join(", ")}`,
    );
  }
  ...
  const config: StripeConfig = {
    ...
    connectApiBaseUrl: Deno.env.get("STRIPE_CONNECT_API_BASE_URL") ||
...
export function getStripeConfigLegacy(): StripeConfig {
  console.warn(
    "⚠️ Using legacy getStripeConfig() - consider updating to getStripeConfig(req) for environment detection",
  );
  ...
  if (!config.connectClientId) {
    console.warn(
      "⚠️ STRIPE_CONNECT_CLIENT_ID not configured - Stripe Connect features may not work",
    );
  }
...
export function buildStripeUrls(config: StripeConfig) {
  return {
    ...
    accounts: (accountId: string) =>
      `${config.apiBaseUrl}/v1/accounts/${accountId}`,
    transfers: () => `${config.apiBaseUrl}/v1/transfers`,
    paymentIntents: (paymentIntentId: string) =>
      `${config.apiBaseUrl}/v1/payment_intents/${paymentIntentId}`,
    sessions: (sessionId: string) =>
      `${config.apiBaseUrl}/v1/checkout/sessions/${sessionId}`,
    ...
    // Webhooks
    webhookEndpoint: (projectUrl: string) =>
      `${projectUrl}/functions/v1/stripe-webhook`,
  };
}
