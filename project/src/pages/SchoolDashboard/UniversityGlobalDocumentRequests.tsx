    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-10">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-[#05294E] mb-1">Document Requests</h2>
        ...
        {!loading && requests.length > 0 && (
          <div className="space-y-4">
            {requests.map((req, index) => (
              <div key={req.id} className="bg-slate-50 rounded-xl p-5 border border-slate-200 hover:border-slate-300 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-start gap-3 mb-3">
                      <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center border border-slate-200">
                        <span className="text-[#05294E] font-bold text-sm">#{index + 1}</span>
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-slate-900 mb-1">{req.title || 'Untitled Request'}</h3>
                        {req.description && (
                          <p className="text-slate-600 text-sm leading-relaxed mb-2">{req.description}</p>
                        )}
                        <div className="flex items-center gap-4 text-xs text-slate-500">
                          <span className="flex items-center gap-1">
                            ...
                            Created: {req.created_at ? new Date(req.created_at).toLocaleDateString() : 'Unknown'}
                          </span>
                  ...
                  {userProfile?.user_id && req.created_by === userProfile.user_id && (
                    <button
                      className="flex items-center gap-2 px-3 py-2 text-slate-600 hover:text-[#05294E] hover:bg-white rounded-lg transition-all duration-200 border border-slate-200 hover:border-slate-300"
                      onClick={() => {
