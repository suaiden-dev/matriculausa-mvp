...
const UrgencyBanner: React.FC<UrgencyBannerProps> = ({ timeLeft }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="bg-rose-600/10 backdrop-blur-md rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-center shadow-2xl border border-rose-500/20 gap-6 sm:gap-10">
...
const VslCosLanding: React.FC = () => {
  ...
  const { trackFormSubmitted } = useFormTracking({ formName: 'vsl_cos_registration' });
  ...
  const [formData, setFormData] = useState(() => {
    try {
      ...
      const saved = sessionStorage.getItem('vsl_cos_form');
  ...
  const [isRegistered, setIsRegistered] = useState(() => sessionStorage.getItem('vsl_cos_registered') === 'true');
  ...
  const [selectedMethod, setSelectedMethod] = useState<'stripe' | 'pix' | 'zelle' | 'parcelow'>(() => (sessionStorage.getItem('vsl_cos_selected_method') as any) || 'stripe');
  const [showZelleCheckout, setShowZelleCheckout] = useState(() => sessionStorage.getItem('vsl_cos_show_zelle') === 'true');
  ...
  useEffect(() => {
    if (formData.full_name || formData.email) sessionStorage.setItem('vsl_cos_form', JSON.stringify(formData));
  }, [formData]);

  useEffect(() => {
    sessionStorage.setItem('vsl_cos_selected_method', selectedMethod);
    sessionStorage.setItem('vsl_cos_show_zelle', String(showZelleCheckout));
  ...
  const handleRegisterAndPay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsRegistered(true); sessionStorage.setItem('vsl_cos_registered', 'true');
    ...
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          metadata: { student_id: sessionData.session.user.id, email: sessionData.session.user.email, full_name: formData.full_name, phone: formData.phone, cpf: formData.cpf, registration_source: 'vsl_cos' }
        })
      });
                          ...
                          <>
                            <div className="space-y-2">
                              ...
                              <div className="relative">
                                ...
                                <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange} className="w-full pl-12 pr-12 py-4 bg-slate-950/50 border border-white/10 rounded-2xl focus:ring-2 focus:ring-rose-500/50 outline-none transition-all text-white" />
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="relative">
                                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" />
                                <input type={showConfirmPassword ? "text" : "password"} name="confirm_password" value={formData.confirm_password} onChange={handleChange} className="w-full pl-12 pr-12 py-4 bg-slate-950/50 border border-white/10 rounded-2xl focus:ring-2 focus:ring-rose-500/50 outline-none transition-all text-white" />
                      ...
                      <div className="mt-8">
                        <div className="flex items-start gap-4 cursor-pointer group" onClick={() => !isRegistered && setFormData(p => ({...p, termsAccepted: !p.termsAccepted}))}>
                           ...
                           <label htmlFor="terms" className="text-sm text-slate-300 font-medium leading-relaxed cursor-pointer select-none">
                              Eu aceito e concordo integralmente com os <button type="button" onClick={(e) => { e.stopPropagation(); handleTermsClick(e); }} className="text-rose-400 hover:text-rose-300 underline font-bold transition-colors">termos e condições</button> do contrato de prestação de serviços.
                           </label>
                        </div>
                      </div>
                    </div>
