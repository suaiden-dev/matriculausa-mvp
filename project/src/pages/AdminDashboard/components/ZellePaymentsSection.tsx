...
interface ZellePaymentsSectionProps {
  zellePayments: PaymentRecord[];
  loadingZellePayments: boolean;
  zelleViewMode: 'grid' | 'list';
  setZelleViewMode: (mode: 'grid' | 'list') => void;
  openZelleProofModal: (paymentId: string) => void;
  openZelleReviewModal: (paymentId: string) => void;
  openZelleNotesModal: (paymentId: string) => void;
}

const ZellePaymentsSection: React.FC<ZellePaymentsSectionProps> = ({
  zellePayments,
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-gray-100 border border-gray-200 rounded-xl p-1">
              <button onClick={() => setZelleViewMode('grid')} className={`flex items-center px-3 py-2 rounded-lg transition-all duration-200 ${zelleViewMode === 'grid' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`} title="Grid view">
                <Grid3X3 className="h-4 w-4" />
              </button>
              <button onClick={() => setZelleViewMode('list')} className={`flex items-center px-3 py-2 rounded-lg transition-all duration-200 ${zelleViewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`} title="List view">
                  ...
                  <div key={payment.id} className="bg-gray-50 rounded-xl p-6 hover:bg-gray-100 transition-colors cursor-pointer border">
                    ...
                    {payment.zelle_status === 'pending_verification' && (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <button onClick={(e) => { e.stopPropagation(); openZelleReviewModal(payment.id); }} className="w-full px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <button onClick={(e) => { e.stopPropagation(); openZelleNotesModal(payment.id); }} className="w-full px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">Add Notes</button>
                    </div>
                  </div>
                ))}
              </div>
                        ...
                        <tr key={payment.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex items-center justify-end space-x-2">
                              ...
                              {payment.zelle_status === 'pending_verification' && (
                                <button onClick={() => openZelleReviewModal(payment.id)} className="text-blue-600 hover:text-blue-900 flex items-center gap-1"><CheckCircle size={16} />Review Payment</button>
                              )}
