        ...
        {feeTypeData.map((fee, index) => (
          <div key={fee.feeType} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-4 h-4 rounded-full ${index === 0 ? 'bg-indigo-500' :  // Selection
                  index === 1 ? 'bg-emerald-500' : // Application
                    index === 2 ? 'bg-rose-500' :    // Placement (Novo)
                      index === 3 ? 'bg-amber-500' :   // Scholarship
                        'bg-purple-500'                  // I-20
                }`}></div>
