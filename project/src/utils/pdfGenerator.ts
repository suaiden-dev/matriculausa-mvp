...
export const generateTermAcceptancePDF = async (data: StudentTermAcceptanceData): Promise<void> => {
  try {
    ...
    if (data.term_content && currentY < pdf.internal.pageSize.getHeight() - 100) {
      ...
      // ✅ Substituir placeholder de nome do estudante no conteúdo do termo
      if (data.term_content) {
        data.term_content = data.term_content.replace(/__________________________|\[Student Name\]/g, data.student_name);
      }
    ...
    const legalText = `This document was automatically generated by MatriculaUSA system on ${new Date().toLocaleString('en-US', { timeZoneName: 'short' })}. ` +
