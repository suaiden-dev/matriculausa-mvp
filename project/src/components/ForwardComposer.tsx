...
const ForwardComposer: React.FC<ForwardComposerProps> = ({ 
  ...
  useEffect(() => {
    console.log('📧 ForwardComposer: useEffect triggered with originalEmail:', {
      id: originalEmail?.id,
...
<div style="border-left: 3px solid #ccc; padding-left: 15px; margin: 20px 0; color: #666;">
  <p style="margin: 0 0 10px 0; font-size: 12px;">
    ...
    <strong>Date:</strong> ${originalEmail.date || new Date().toLocaleString()}<br>
  ...
  <div style="border-top: 1px solid #eee; padding-top: 10px;">
    ${originalEmailContent}
        ...
        setBody(forwardHtml);
        setIsHtmlMode(true);
        console.log('📧 ForwardComposer: Set HTML mode with formatted forward');
      } else {
        // Se é texto simples
        const forwardTemplate = `\n\n---------- Forwarded message ----------\nFrom: ${originalEmail.from}\nDate: ${originalEmail.date || new Date().toLocaleString()}\nSubject: ${originalEmail.subject}\n\n${originalEmailContent}`;
        setBody(forwardTemplate);
        setIsHtmlMode(false);
        console.log('📧 ForwardComposer: Set text mode with simple forward');
      }
    }
  }, [originalEmail]);
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-[#D0151C] rounded-xl flex items-center justify-center">
              <Send className="h-5 w-5 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-900">Forward Email</h2>
              <p className="text-sm text-slate-600">Forward this message to someone else</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#D0151C] focus:border-transparent"
            ...
            <div className="flex items-center space-x-4">
              <label className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded border-slate-300 text-[#D0151C] focus:ring-[#D0151C]"
                />
            ...
            <div>
              ...
              {isHtmlMode ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <div>
                    <textarea
                      ...
                      rows={12}
                      className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#D0151C] focus:border-transparent resize-none font-mono text-sm"
                ...
                <textarea
                  ...
                  onChange={(e) => setBody(e.target.value)}
                  rows={12}
                  className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#D0151C] focus:border-transparent resize-none"
            ...
            <div>
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-[#D0151C] file:text-white hover:file:bg-[#b01319] transition-colors"
                />
        ...
        <div className="flex items-center justify-between p-6 border-t border-slate-200">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-[#D0151C] text-white rounded-xl hover:bg-[#b01319] disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold"
            >
              {isSending ? (
                <>
