...
export function useAgencyCommissionsQuery(userId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Se a tabela não existir, retornar erro ou estiver vazia, geramos as comissões dinamicamente
      if (error || list.length === 0) {
        const { data: aaData } = await supabase
          .from('affiliate_admins')
          .select('commission_rules, system_type')
          .eq('id', affiliateAdminId)
          .maybeSingle();

        const rules = aaData?.commission_rules || {};
        const isSimplified = aaData?.system_type === 'simplified';

        const { data: profiles } = await supabase
          .rpc('get_affiliate_admin_profiles_with_fees', { admin_user_id: userId });

        if (profiles && profiles.length > 0) {
          const generatedComms: any[] = [];
          ...
          profiles.forEach((p: any) => {
            ...
            const isTransferInactiveVisa = p.student_process_type === 'transfer' && p.visa_transfer_active !== true;
            const isI20Applicable =
              p.student_process_type === 'initial' ||
              p.student_process_type === 'change_of_status' ||
              isTransferInactiveVisa;
            const isPlacementApplicable = !!p.placement_fee_flow;
            const isReinstatementApplicable = p.student_process_type === 'transfer' && p.visa_transfer_active !== true;

            const placementOrScholarshipPaid = isPlacementApplicable
              ? !!p.is_placement_fee_paid
              : !!p.is_scholarship_fee_paid;

            const i20Paid = !!p.has_paid_i20_control_fee || !!p.has_paid_ds160_package || !!p.has_paid_i539_cos_package;

            if (p.has_paid_selection_process_fee) {
              const commAmount = calculateRuleComm('selection_process', selectionFeeAmount);
              if (commAmount > 0) {
                generatedComms.push({
                  id: `v_${p.profile_id}_selection`,
                  created_at: p.created_at || new Date().toISOString(),
                  fee_type: 'selection_process',
                  amount: commAmount,
                  student_id: p.profile_id,
                  affiliate_code: p.seller_referral_code || '—',
                  payment_amount: selectionFeeAmount,
                  status: 'active'
                });
              }
            }

            if (p.is_application_fee_paid) {
              const commAmount = calculateRuleComm('application', applicationFeeAmount);
              if (commAmount > 0) {
                generatedComms.push({
                  id: `v_${p.profile_id}_application`,
                  created_at: p.created_at || new Date().toISOString(),
                  fee_type: 'application',
                  amount: commAmount,
                  student_id: p.profile_id,
                  affiliate_code: p.seller_referral_code || '—',
                  payment_amount: applicationFeeAmount,
                  status: 'active'
                });
              }
            }
            ...
            if (placementOrScholarshipPaid) {
              const ruleKey = isPlacementApplicable ? 'placement' : 'scholarship';
              const commAmount = calculateRuleComm(ruleKey, scholarshipFeeAmount);
              if (commAmount > 0) {
                generatedComms.push({
                  id: `v_${p.profile_id}_${ruleKey}`,
                  created_at: p.created_at || new Date().toISOString(),
                  fee_type: ruleKey,
                  amount: commAmount,
                  student_id: p.profile_id,
                  affiliate_code: p.seller_referral_code || '—',
                  payment_amount: scholarshipFeeAmount,
                  status: 'active'
                });
              }
            }

            if (isReinstatementApplicable && p.has_paid_reinstatement_package) {
              const commAmount = calculateRuleComm('reinstatement', 500);
              if (commAmount > 0) {
                generatedComms.push({
                  id: `v_${p.profile_id}_reinstatement`,
                  created_at: p.created_at || new Date().toISOString(),
            ...
            if (isI20Applicable && i20Paid) {
              const commAmount = calculateRuleComm('i20_control', i20FeeAmount);
              if (commAmount > 0) {
                generatedComms.push({
                  id: `v_${p.profile_id}_i20_control`,
                  created_at: p.created_at || new Date().toISOString(),
                  fee_type: 'i20_control',
                  amount: commAmount,
                  student_id: p.profile_id,
                  affiliate_code: p.seller_referral_code || '—',
                  payment_amount: i20FeeAmount,
                  status: 'active'
                });
              }
            }
          ...
          generatedComms.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
          list = generatedComms;
        }
      }
