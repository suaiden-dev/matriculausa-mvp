...
interface AIProvider {
  name: string;
  analyzeEmail: (email: any) => Promise<EmailAnalysis>;
  generateResponse: (email: any, analysis: EmailAnalysis) => Promise<string>;
}
...
class GeminiService implements AIProvider {
  ...
  async analyzeEmail(email: any): Promise<EmailAnalysis> {
    const prompt = `
...
`;
  ...
  async generateResponse(email: any, analysis: EmailAnalysis): Promise<string> {
    const prompt = `
...
export class AIService {
  ...
  constructor(provider: 'gemini' | 'openai' | 'mock' = 'mock', apiKey?: string) {
  ...
  async processEmail(email: any): Promise<{ analysis: EmailAnalysis; response?: string }> {
